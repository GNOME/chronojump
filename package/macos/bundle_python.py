...
OTOOL_LIB_REGEX = re.compile(r"(/Library/Frameworks/Python.framework/[^\(^\)]+) \(.*\)")
...
def run_install_name_tool(src_lib, referenced_path):
    ...
    dep_lib_name = os.path.realpath(referenced_path).replace("/Library/Frameworks/Python.framework/", "")
...
def traverse_directory(path):
    for root, dirs, files in os.walk(path):
        ...
        for file_name in files:
            if file_name.lower() == f"python" or file_name.lower().endswith(f".dylib") or file_name.lower().endswith(f".a") or file_name.lower().endswith(f".so"):
                #print(f"File: {os.path.join(root, file_name)}")
                refresh_libs(os.path.join(root, file_name))


parser = argparse.ArgumentParser(description='Bundle Python binaries.')
