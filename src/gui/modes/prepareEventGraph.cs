...
public abstract class PrepareEventGraphTest
{
	...
	protected void personHistoricalBest (string param)
	{
		...
		historicalExStr = string.Format (
			...
			string.Format ("{0} {1} ({2})",
					...
					historicalExUnits,
					printHistoricalDate (dtr.date));
					//UtilDate.GetDatetimePrint (UtilDate.FromFile (dtr.date)));
		//historicalExStr += personHistoricalSpecialMessageForThisMode ();
	}
	...
	protected virtual string printHistoricalDate (string dateStr) //this on encoder is already well fromatted
	{
		return UtilDate.GetDatetimePrint (UtilDate.FromFile (dateStr));
	}
	/*
	protected virtual string personHistoricalSpecialMessageForThisMode ()
	{
		return "";
	}
	*/
...
public class PrepareEventGraphEncoderSession : PrepareEventGraphTest
{
	...
	private double selectedSetWeight; //gravitatory
	private EncoderConfiguration selectedSetEconf; //inertial
	...
	private int currentExerciseID; //TODO: more to parent
	private string currentExerciseNameAndConditions;
	private int exerciseIDForGraph;
	...
	public PrepareEventGraphEncoderSession (
			...
			int limit,
			int currentExerciseID, string currentExerciseNameAndConditions,
			int selectedSetID,
			double selectedSetWeight, // gravitatory
			EncoderConfiguration selectedSetEconf, // inertial
			Constants.Modes mode, bool exerciseAll)
	{
		this.sessionID = sessionID;
		this.currentPersonID = currentPersonID;
		this.currentPersonName = currentPersonName;
		...
		this.currentExerciseID = currentExerciseID;
		this.currentExerciseNameAndConditions = currentExerciseNameAndConditions;
		this.selectedSetID = selectedSetID;
		this.selectedSetWeight = selectedSetWeight;
		this.selectedSetEconf = selectedSetEconf;
		this.mode = mode;
		this.exerciseAll = exerciseAll;
		...
		rowsAtSQL = SqliteEncoder.SelectList (true, -1, personIDForGraph, sessionID, encoderGI,
				exerciseIDForGraph, "curve", EncoderSQL.Eccons.ALL,
				"", 	//lateralityEnglish
		...
		if (selectedSetID >= 0)
		{
			ArrayList linkedReps = SqliteEncoderSignalCurve.SelectSignalCurve (
					true, selectedSetID, -1, -1, -1);	//DBopened, signal, curve, msStart, msEnd
		...
		string param = Constants.GetEncoderVariablesCaptureAsSQLField (encoderVariablesCapture);
		boxplotsDo (param);
		personHistoricalBest (param);
	...
	protected override void selectEventFromSQL ()
	{
		...
		List<List<EncoderSQL>> eSQL_ll = sqliteEncoder.SelectSetsAndRepsLList (true, currentPersonID, sessionID, encoderGI,
				currentExerciseID, selectedSetID);
	...
	protected override List<double> boxplotSelectPerson (string param)
	{
		return SqliteEncoder.SelectList (true, param, -1, personIDForGraph, sessionID, encoderGI,
				exerciseIDForGraph, "curve", EncoderSQL.Eccons.ALL,
				"", 	//lateralityEnglish
				false, orderBy, encoderVariablesCapture, 	// onlyActive
	...
	protected override List<double> boxplotSelectSession (string param)
	{
		return SqliteEncoder.SelectList (true, param, -1, -1, sessionID, encoderGI,
				exerciseIDForGraph, "curve", EncoderSQL.Eccons.ALL,
				"", 	//lateralityEnglish
				false, orderBy, encoderVariablesCapture, 	// onlyActive
				false, 	//orderRepsByPosInSet
	...
	public override bool personHistoricalBestHaveData ()
	{
		...
		string andSecondaryStr = " AND signalOrCurve = 'curve'";
		if (mode == Constants.Modes.POWERGRAVITATORY)
		{
			if (selectedSetWeight >= 0)
				andSecondaryStr += " AND extraWeight = " + Util.ConvertToPoint (selectedSetWeight);
		}
		else if (mode == Constants.Modes.POWERINERTIAL)
		{
			andSecondaryStr += " AND encoderConfiguration = '" +
				selectedSetEconf.ToStringOutput (EncoderConfiguration.Outputs.SQL) + "'";
		}

		SqliteBest sb = new SqliteBest ();
		return sb.HaveEventsInOtherSessions (true, sessionID, currentPersonID,
					Constants.EncoderTable, "", currentExerciseID, Constants.EncoderExerciseTable, andSecondaryStr);
	}
	public override SqliteStruct.DateTypeResult personHistoricalBestGetData (string param)
	{
		SqliteEncoder se = new SqliteEncoder ();
		// future1 is TXT (need to CAST) and is ambiguous encoder/encoderExercise. passing "encoder."
		string paramStr = string.Format ("CAST ({0}.{1} as FLOAT)", Constants.EncoderTable, param);
		...
		SqliteBest sb = new SqliteBest ();
		SqliteStruct.DateTypeResult dtr = sb.Select_MAX_EventsOfAType (true, -1, currentPersonID,
				Constants.EncoderTable, "", currentExerciseID,
				Constants.EncoderExerciseTable,
				paramStr, andSecondaryStr);

		if (selectedSetWeight >= 0)
			dtr.type += string.Format (" ({0} Kg CON)", selectedSetWeight);

		return dtr;
	}
	protected override string printHistoricalDate (string dateStr)
	{
		return (dateStr);
	}
	/*
	protected override string personHistoricalSpecialMessageForThisMode ()
	{
		if (selectedSetWeight >= 0)
			return string.Format ("\t{0} Kg", selectedSetWeight);
		else
			return "";
	}
	*/
