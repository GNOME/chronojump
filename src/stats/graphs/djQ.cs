...
public class GraphDjQ : StatDjQ
{
	...
	public GraphDjQ (ArrayList sessions, int newPrefsDigitsNumber, string jumpType, bool showSex, int statsJumpsType, int limit) 
	{
		...
		if(this.operation == "MAX") {
			CurrentGraphData.GraphTitle = Catalog.GetString("MAX values of Q Index") 
				+ " (tv/tc) " + Catalog.GetString("in") + " " + mySessions; 
		} else {
			CurrentGraphData.GraphTitle = Catalog.GetString("AVG values of Q Index") 
				+ " (tv/tc) " + Catalog.GetString("in") + " " + mySessions; 
		}
