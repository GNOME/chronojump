...
public class Questionnaire
{
	...
	// autogenerated arithmetic (level 0-3)
	public Questionnaire (int n, int qDuration, int level)
	{
		this.n = n;
		this.qDuration = qDuration;

		qa_l = new List<QuestionAnswers> (); //blank the default list
		QuestionAnswerArithmetic qaa;

		if (level == 3)
			qaa = new QuestionAnswerArithmeticGodLevel ();
		else
			qaa = new QuestionAnswerArithmetic (level);

		for (int i = 0; i < n; i ++)
			qa_l.Add (qaa.GenerateQA ());

		questionType = qaa.DifficultyName + "    ";
