    ...
    public Session Select(string myUniqueID)
    {
        ...
        else
        {
            ...
            Session mySession = new Session(values[0],
                                            values[1], values[2], UtilDate.DateFromSQL(values[3]),
	...
	private static List<Session> selectDo(bool dbconOpened, SQLiteCommand mydbcmd)
	{
		...
		while(reader.Read())
		{
			Session session = new Session(
					...
					reader[1].ToString(),
					reader[2].ToString(),
					UtilDate.DateFromSQL(reader[3].ToString()),
		...
		while(reader.Read()) {
			if(commentsDisable) {
				myArray.Add (reader[0].ToString() + ":" + reader[1].ToString() + ":" +
						reader[2].ToString() + ":" + UtilDate.DateFromSQL(reader[3].ToString()).ToShortDateString() );
			} else {
				myArray.Add (reader[0].ToString() + ":" + reader[1].ToString() + ":" +
						reader[2].ToString() + ":" + UtilDate.DateFromSQL(reader[3].ToString()).ToShortDateString() + ":" +
						reader[4].ToString() );
