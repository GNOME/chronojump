...
public class GraphDjIndex : StatDjIndex
{
	...
	public GraphDjIndex (ArrayList sessions, int newPrefsDigitsNumber, bool showSex, int statsJumpsType, int limit) 
	{
		...
		valuesSchemaIndex.Add ("true"); //djIndex
		...
		if(sessions.Count > 1) {
			this.graphTitle = "DjIndex " + operation + Catalog.GetString(" values chart of multiple sessions");
		} else {
			this.graphTitle = "DjIndex " + operation + Catalog.GetString(" values chart of single session");
